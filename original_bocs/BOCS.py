...
def BOCS(inputs, order, acquisitionFn):
	...
	for t in range(n_iter):
		...
		if acquisitionFn == 'SA':
			...
			stat_model = lambda x: LR.surrogate_model(x, alpha_t) + penalty(x)    # PENALTY here
...
def simulated_annealing(objective, inputs):
	...
	print('SA - (2.) start:   ...  kk in range(n_rand_vector)')
	for t in range(n_iter):
		...
		#print('type(new_obj): ', type(new_obj))
		#print('type(old_obj): ', type(old_obj))
		#print('new_obj.shape: ', new_obj.shape)
		#print('old_obj.shape: ', old_obj.shape)
		if (new_obj < old_obj) or (np.random.rand() < np.exp( (old_obj - new_obj)/T )): # LEGACY
			old_x   = new_x
			old_obj = new_obj
...
def sdp_relaxation(alpha, inputs):
	...
	print('SDP - (2.) start:   ...  kk in range(n_rand_vector)')
